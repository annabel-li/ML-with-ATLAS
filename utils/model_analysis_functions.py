...
as well as comparing performance between parameterizations with metrics 
such as Mean Percent Error (MPE) and Mean Absolute Error (MAE). Many of them rely on setting trace to True during 
the hls4ml conversion - see the PTQ and QAT folders for more details on how to implement this."""
...
#Purpose: adjusts the Keras and hls4ml trace dictionaries so they contain the same keys; enables easy pairwise comparison without running into loop errors. 
def refine_dicts(
        keras_trace: dict, 
        hls_trace: dict
):

    keras_keys = set(keras_trace.keys())
    hls_keys = set(hls_trace.keys())

    # Find keys that are common to both dictionaries
    common_keys = keras_keys.intersection(hls_keys)
    
    keys_to_keep = set()

    # From the common keys, keep only those where the shapes match
    for key in common_keys:
        if keras_trace[key].shape == hls_trace[key].shape:
            keys_to_keep.add(key)
        else:
            print(f"Ignoring common key '{key}' due to shape mismatch: "
                  f"Keras shape={keras_trace[key].shape}, HLS shape={hls_trace[key].shape}")

    # Create new dictionaries with only the keys that should be kept
    refined_keras_trace = {k: keras_trace[k] for k in keys_to_keep}
    refined_hls_trace = {k: hls_trace[k] for k in keys_to_keep}

    # For debugging, show which keys were discarded
    discarded_keras = keras_keys - keys_to_keep
    if discarded_keras:
        print("Keys discarded from keras_trace:", sorted(list(discarded_keras)))
    
    discarded_hls = hls_keys - keys_to_keep
    if discarded_hls:
        print("Keys discarded from hls_trace:", sorted(list(discarded_hls)))

    return refined_keras_trace, refined_hls_trace
